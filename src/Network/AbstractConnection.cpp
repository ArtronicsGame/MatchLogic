...
void AbstractConnection::createServer(){
    int addrlen, opt = 1;
...
void AbstractConnection::readSync(){
    if(!reading){
...
void AbstractConnection::closeOne(int sock){
    FD_CLR(sock, &fds);
    close(sock);
...
void AbstractConnection::closeAll(){
    reading = false;
}

void AbstractConnection::sendTCP(int sock, string msg){
    send(sock, msg.c_str(), msg.length() + 1, 0);
}

void AbstractConnection::broadcastTCP(string msg){
    for(int sock: clients)
        sendTCP(sock, msg);
}

void AbstractConnection::broadcastTCP(json msg){
    stringstream ss;
    ss << msg << endl;
    for(int sock: clients)
        sendTCP(sock, ss.str());
...
void AbstractConnection::sendUDP(sockaddr* addr, int len, char* buf, int m_len){
    sendto(udp, buf, m_len, 0, addr, len);
}

void AbstractConnection::sendUDPPacket(sockaddr* addr, int len, UDPPacket packet){
    char buf[1024];
